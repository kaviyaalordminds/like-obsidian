...
export function CommandPalette() {
  ...
  const items = [
    ...
    { icon: LayoutDashboard, label: 'Open canvas', action: () => setMainView('canvas') },
    { icon: Hash, label: 'Open tags', action: () => setMainView('tags') },
    ...
    { icon: Focus, label: 'Toggle focus mode', action: () => useUIStore.getState().toggleFocusMode() },
    { icon: Minimize2, label: 'Toggle zen mode', action: () => useUIStore.getState().toggleZenMode() },
